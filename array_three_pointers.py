...
A Simple Solution is to run three nested loops to consider all triplets from A, B and C. Compute the value of max(abs(A[i] – B[j]), abs(B[j] – C[k]), abs(C[k] – A[i])) for every triplet and return minimum of all values. Time complexity of this solution is O(n^3)
'''
def min_max_diff_three_pointers(arr1, arr2, arr3):
    ...
    Let's say the heads of X, Y, and Z are 10, 5, and 20, so that y=5 is the min.
    Consider all tuples (x, 5, z). Since X is sorted we know that x >= 10 for all x,
    and likewise all z >= 20. So now we know any solution involving y=5 has the form (x >= 10, 5, z >= 20).
    Since y=5 is less than 10 and 20, we clearly know the optimal solution for that
    form has to be (10, 5, 20), which we just evaluated. Therefore, since we have proven
    that we've already considered the optimal solution for y=5, we can advance the Y pointer
    beyond y=5 to find more optimal solutions over (X, Y, Z). It's also pretty easy to
    convince ourselves that the algorithm terminates in linear time, since we always exclude
    ...
    min_diff = float('inf')
    while i < len(arr1) and j < len(arr2) and k < len(arr3):
        cur_diff = max(abs(arr1[i]-arr2[j]), abs(arr2[j]-arr3[k]), abs(arr3[k]-arr1[i]))
