"""
Biconnected component is one which has 2 or more way of connecting to each of the
vertices (bi) and therefore doesnot have any articulation points.
Threrfore conditions to check:
    1. Entire graph is connected.
    2. Graph doesnot have articulation point.
"""
...
class Graph:
    ...
    def is_biconnected(self, node_map):
        """
        ...
        """
        if self.dfs(node_map[0])
        for node in node_map:
            if node.visited_time is None:
                return False
        return True
