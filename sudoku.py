...
class Solution(object):
    def is_valid(self, board, cur_row, cur_col, cur_num):
        for index in xrange(9):
            if board[cur_row][index] == cur_num: return False
            if board[index][cur_col] == cur_num: return False
            try:
                if board[index/3][index%3] == cur_num: return False
    ...
    def solveSudoku(self, board):
        ...
        for cur_row in xrange(len(board)):
            for cur_col in xrange(len(board)):
                if board[cur_row][cur_col] == '.':
                    for cur_num in xrange(1, 10):
